...
async def bronze(ctx):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    embed=discord.Embed(title="Bronze gratis",color=10181046, description=f"**Ponte .gg/socialmediaboosting en el status, y se te pondra automaticamente**")
    await ctx.send(embed=embed)
...
async def on_member_update(before, after):
    role_id = 971142617272295476
    role = get(before.guild.roles, id=role_id)
    if '.gg/socialmediaboosting' in str(before.activities):
      if '.gg/socialmediaboosting' in str(after.activities):
        pass
      else:
        await after.remove_roles(role)
        channel = bot.get_channel(974441848334581820)
...
async def clear(ctx):
    ...
    embed = DiscordEmbed(title='Social Media Boosting', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}clear**'), color='9B59B6')
...
async def tfollow(ctx, arg):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    json_object = json.loads(config_file)
    webhook = DiscordWebhook(url='')
    embed = DiscordEmbed(title='Social Media Boosting', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}tfollow -> {arg}**'), color='9B59B6')
    webhook.add_embed(embed)
    response = webhook.execute()
    genchannel =json_object['bot_config']["twitch_channel"]
        ...
        for role_name in role_config:
            ...
            if role_id in ctx.author.roles:
                ...
                if num_lines < follow_count:
                    ...
                    embed=discord.Embed(color=10181046, description=f"⇒ Añadiendo **{num_lines}** seguidores a **{arg}**")
                    await ctx.send(embed=embed)
                    
                    caunt_to_follow = num_lines
                else:
                    
                    embed=discord.Embed(color=10181046, description=f"⇒ Añadiendo **{follow_count}** seguidores a **{arg}**")
                    await ctx.send(embed=embed)
...
async def ttroll(ctx, arg1):
 if ctx.channel.type != discord.ChannelType.private:
    config_file = get_config()
    json_object = json.loads(config_file)
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/974639508437999626/ImKzicqKVKIuLZkRpY_rTVKlZtNzJYgWa4AN-W83rHBUmy1LrUWW4sNymVZpodAZOGT7')
...
async def thost(ctx, arg1):
    if  discord.utils.get(ctx.guild.roles, name='Premium +') in ctx.author.roles or discord.utils.get(ctx.guild.roles, name='Premium') in ctx.author.roles or discord.utils.get(ctx.guild.roles, name='Diamond') in ctx.author.roles or discord.utils.get(ctx.guild.roles, name='Gold') in ctx.author.roles:
        config_file = get_config()
        json_object = json.loads(config_file)
        webhook = DiscordWebhook(url='https://discord.com/api/webhooks/974639508437999626/ImKzicqKVKIuLZkRpY_rTVKlZtNzJYgWa4AN-W83rHBUmy1LrUWW4sNymVZpodAZOGT7')
        embed = DiscordEmbed(title='Twitch Follower V4', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}thost -> {arg1}**'), color='9B59B6')
        webhook.add_embed(embed)
        response = webhook.execute()
        genchannel = json_object['bot_config']["twitch_channel"] 
        genchannel2 = json_object['bot_config']["twitch_channel_2"]
        if ctx.channel.id == int(genchannel) or ctx.channel.id == int(genchannel2): 
            ...
            for role_name in role_config:
                ...
                if role_id in ctx.author.roles:    
                    ...
                    embed=discord.Embed(color=10181046, description=f"**{arg1}** getting hosted, wait 1/1.30 minute to sent!")
                    await ctx.send(embed=embed)
                    try:
                        for i in range(x):
...
@bot.command()
async def twhisper(ctx,user,message):
   if ctx.channel.id != 963049710124404785 or ctx.channel.id != 963049710124404785:
      embed = discord.Embed(color=10181046 ,description='**You must do commands in <#963049710124404785>**')
      await ctx.send(embed=embed)
   else:
     embed = discord.Embed(color=10181046 ,description=f'? Sending  whispers to **{user}** ')
     await ctx.send(embed=embed)
     webhook = DiscordWebhook(url='https://discord.com/api/webhooks/974639508437999626/ImKzicqKVKIuLZkRpY_rTVKlZtNzJYgWa4AN-W83rHBUmy1LrUWW4sNymVZpodAZOGT7')
     embed = DiscordEmbed(title='Twitch Follower V4', description=(f'**{ctx.author} | {ctx.author.id} -> {bot.command_prefix}twhisper -> {user}**'), color='9B59B6')
     webhook.add_embed(embed)
     response = webhook.execute()
     b = get_id(user)
     for i in range(5):
      tokens = ['i0q54csadnzieymkby91rjwjhz8zzw','723v5o6k287evz10fb4yl5z3xjldfd']
      headers = {
        'Authorization': f'OAuth {random.choice(tokens)}',
        'Client-Id': 'kimne78kx3ncx6brgo4mv6wki5h1ko'
      }
      payload = [{"operationName":"SendWhisper","variables":{"input":{"message":f"{message}","nonce":"35cYaCq1ZkkEiYL8e8pUuXzVzyuEYJdod4","recipientUserID":f"{b}"}},"extensions":{"persistedQuery":{"version":1,"sha256Hash":"3bbd599e7891aaf3ab6a4f5788fd008f21ad0d64f6c47ea6081979f87e406c08"}}}]
      r = requests.post('https://gql.twitch.tv/gql',json=payload,headers=headers)
